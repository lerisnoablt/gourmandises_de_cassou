...
function updateCartBadges(){
  const text = `Panier · ${formatPrice(getTotal())}`;
  document.querySelectorAll("[data-cart-total]").forEach(el => el.textContent = text);
}
...
function renderThumb(item){
  ...
  return `<img class="item-thumb" src="${item.thumb || 'assets/images/hero.png'}" alt="${item.name}">`;
}
